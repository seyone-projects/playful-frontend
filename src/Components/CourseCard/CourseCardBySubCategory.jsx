...
function CourseCard() {
  ...
  const fetchSubCategoryDetails = async () => {
    try {
      setIsLoading(true);
      const response = await GetById(subCategoryId);
      if (response?.subCategory) {
        setSubCategoryDetails(response.subCategory);
      } else {
  ...
  const fetchCourses = async (page = 1) => {
    try {
      const response = await GetBySubCategoryId(subCategoryId, page, itemsPerPage);
  ...
  return (
    <div className="container">
      <h2 className="category-card-title text-center my-4">
        Browse Courses By {subCategoryDetails?.name}
      </h2>
      <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
        {courses.length > 0 ? (
          ...
          <p className="text-center mt-4">No courses are available for {subCategoryDetails?.name}.</p>
        )}
      </div>
