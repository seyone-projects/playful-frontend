...
const CategoryCard = () => {
  ...
  //fetch categories by sectionId with active status only
  const fetchCategoryBySectionId = async () => {
    try {
      setIsLoading(true);
      const response = await GetBySectionId(id);
      setCategories(response.categories?.filter(cat => cat.status === 'active') || []);
      // load subcategories of first active category initially
      if (response.categories && response.categories.length > 0) {
        const firstActiveCategory = response.categories.find(cat => cat.status === 'active');
  ...
  const fetchSubCategories = async (categoryId, page = 1) => {
    try {
      setIsLoading(true);
      const response = await GetByCategoryId(categoryId, page, itemsPerPage);
  ...
  const handleSubPageChange = (page) => {
    const categoryId = categories[activeCategoryIndex]?._id;
    if (categoryId && page >= 1 && page <= totalSubPages) {
  ...
  return (
    <div className="category-card-section container my-5">
      ...
      {/* Tabs */}
      <div className="category-card-tabs d-flex justify-content-center mb-4">
        {categories.map((category, index) => (
          <button
            key={index}
            className={`category-card-tab-btn btn btn-light mx-2 ${activeCategoryIndex === index ? "category-card-tab-active" : ""
              }`}
            onClick={() => {
              setActiveCategoryIndex(index);
              setCurrentSubPage(1);
              fetchSubCategories(category._id, 1);
            }}
          >
            {category.name}
          </button>
